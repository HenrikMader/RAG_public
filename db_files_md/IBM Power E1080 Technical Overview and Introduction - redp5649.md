...
| 2.7.2 2.8 External                                                                                        | PCIe Gen3 I/O Expansion Drawer . . . . . . . . . . . . . . . . . . . disk subsystems . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . | 106       |
...
|----------------|------------------------|--------------|-----------------------------------------------|------------------------------------------------------|
|           6653 | IEC 309, 3P+N+G, 16 A  | 60 Amps      | Twosystemnodes and 1 I/O expansion drawer     |                                                    8 |
|           6489 | IEC309 3P+N+G, 32 A    | 60 Amps      | Twosystemnodes and 1 I/O expansion drawer     |                                                    8 |
...
|           6492 | IEC 309,2P+G, 60 A     | 60 Amps      | Twosystemnodeand1 I/O expansion drawer        |                                                    4 |
|           6491 | IEC 309, P+N+G, 63 A   | 60 Amps      | Twosystemnodes and 1 I/O expansion drawer     |                                                    8 |
...
For an initial Power E1080 order with the IBM configurator (e-config), HMC virtual appliance can be found by selecting add software → Other System Offering s (as product selections) and then:
...
3   A-busses and X-busses provide SMP fabric ports that are used between central electronic complex (CEC) drawers or within CEC drawers respectively.
...
| Characteristics           | Power10                                                                                                               | Power9                                                                                                  | Power8                                                                                             |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| L3 Cache                  | 8 MB of L3 cache per core with each core having access to the full 120 MB of L3 cache, on-chip high-efficiency SRAM a | 10 MB of L3 cache per core with each core having access to the full 120 MB of L3 cache, on-chip eDRAM b | 8 MB of L3 cache per core with each core havingaccess to the full 96 MB of L3 cache, on-chip eDRAM |
...
|                | DDR4 DDIMMS   | DDR4 DDIMMS                             | DDR5 DDIMMS   | DDR5 DDIMMS                             |
|----------------|---------------|-----------------------------------------|---------------|-----------------------------------------|
| DDIMM capacity | Clock speed   | Maximum theoretical bandwidth per SCM a | Clock speed   | Maximum theoretical bandwidth per SCM b |
...
|-----------------|-----------------------------|----------------|---------------|---------|
| P0-C0           | PCIeGen4x16or PCIe Gen 5 x8 | P0/1           | 1             | Yes     |
| P0-C1           | PCIeGen4x16or PCIe Gen 5 x8 | P0/0           | 2             | Yes     |
| P0-C2           | PCIeGen4x16or PCIe Gen 5 x8 | P2/1           | 3             | Yes     |
...
| P0-C5           | PCIeGen4x16or PCIe Gen 5 x8 | P3/0           | 6             | Yes     |

| Location code   | Description                 | Socket / PHB   |   Slot number | SRIOV   |
|-----------------|-----------------------------|----------------|---------------|---------|
| P0-C6           | PCIeGen4x16or PCIe Gen 5 x8 | P1/1           |             7 | Yes     |
| P0-C7           | PCIeGen4x16or PCIe Gen 5 x8 | P1/0           |             8 | Yes     |
...
| ECCY           |        | 10MAOCPairfor PCIe Gen 3 Expansion Drawer                           |              | Both         |
| ECCZ           |        | 20MAOCPairfor PCIe Gen 3 Expansion Drawer                           |              | Both         |
...
| EJ11           | 57B4   | PCIe Gen 3 LP SAS Tape/DVDAdapterQuad-port 6 Gb x8 |     0 |    32 | AIX, IBM i, and Linux | Both         |
...
| EJ2C           | 57F2   | PCIe3LP12Gbx8SASTape HBA Adapter                               |     0 |    32 | IBM i                 | Both         |
...
|----------------|--------|-------------------------------|-------|-------|------------------------|-----------|
| ESLS           | 78D1   | EXP24SX SAS Storage Enclosure |     0 |   168 | AIX, IBM i, and Linuxf | Both      |
...
5. All slots support EEH.
6. All PCIe slots can be serviced with the power on.
7. All six slots in a PCIe4 6-slot Fanout Module support Single Root I/O Virtualization (SR-IOV) shared mode.
8. Only four Feature Code EC2S, Feature Code EC2U, or Feature Code EC72 Adapters Can Be In SR-IOV Mode Simultaneously Per 6-slot Fanout module.
...
|--------------------|-------------------------|-------------------------|-------------------------|-------------------------|
| System nodes       | Maximum #EMX0 drawers   | PCIe Gen 3, x16         | PCIe Gen 3, x8          | Total PCIe Gen 3        |
...
| SUSE Linux EnterpriseServer 12 SP5 | Power9           |                   1536 | 64 TB            | 32 TB                     |
| SUSE Linux EnterpriseServer 15 SP3 | Power10          |                   1920 | 64 TB            | 64 TB                     |
...
```
$ lscfg -vpl sysplanar0 | grep -p "System Firmware" System Firmware: ... Microcode Image.............SV860_138 SV860_103 SV860_138 Microcode Level.............FW860.42 FW860.30 FW860.42 Microcode Build Date........20180101 20170628 20180101 Microcode Entitlement Date..20190825 Hardware Location Code......U8284.22A.XXXXXXX-Y1 Physical Location: U8284.22A.XXXXXXX-Y1
```
...
```
$ lscfg -vpl sysplanar0 |grep -p "System Firmware" System Firmware: ... Microcode Image.............SV860_138 SV860_103 SV860_138 Microcode Level.............FW860.42 FW860.30 FW860.42 Microcode Build Date........20180101 20170628 20180101 Update Access Key Exp Date..20190825 Hardware Location Code......U8284.22A.XXXXXXX-Y1 Physical Location: U8284.22A.XXXXXXX-Y1
```
...
2. -IBM Service website
3. -Removable Media
4. -FTP Site
5. -Secure FTP Site
6. -Import Staging Location
7. -Mount Point
...
From an IBM i perspective, an example is the ability to run SQL queries against the integrated IBM Db2® database that comes built into the IBM i platform, manage object authorities, and other tasks. All of these things mean something to an AIX administrator or IBM i administrator.
...
1 Source: Thales Data Threat Report - Global Edition:
...
By using a combination of software and hardware innovation, Power E1080 can meet the model performance, response time, and throughput KPIs of databases and applications that are infused with AI.
...
- IBM Power Systems Private Cloud with Shared Utility Capacity: Featuring Power Enterprise Pools 2.0, SG24-8476
- IBM Power System S822LC for High Performance Computing Introduction and Technical Overview , REDP-5405
- IBM Power Systems S922, S914, and S924 Technical Overview and Introduction Featuring PCIe Gen 4 Technology , REDP-5595
- IBM PowerVM Best Practices , SG24-8062
...
| AES      | Advanced Encryption Standard                | FSI     |
|----------|---------------------------------------------|---------|
| AI       | artificial intelligence                     | FSP     |
| AME      | Active Memory Expansion                     | GCC     |
| AOC      | Active Optical Cable                        | GTps    |
| API      | application programming interface           | HDD     |
| ARI      | analog rack interface                       | HHHL    |
| ASMI     | Advanced System Management Interface        | HMC HNV |
| BMC      | baseboard management controller             | HPT     |
| BSC      | blind-swap cassette                         | IaaS    |
| CBU      | Capacity Backup                             | IBM     |
| CEC      | central electronic complex                  |         |
| CMOS     | complimentary metal-oxide-semiconductor     | IPS ISA |
| CoD      | Capacity on Demand                          | ISV     |
| CRU      | customer-replaceable unit                   | KVM     |
| CVE      | Common Vulnerability Exposure               | LaMa    |
| DDIMM    | differential dual inline memory module      | LP      |
| DDR4     | Double Data Rate 4                          | LPAR    |
| DDR5     | Double Data Rate 5                          | LPD     |
| DEXCR    | Dynamic Execution Control Register          | LPM MCU |
| DFP      | decimal floating-point                      | MES     |
| DLPAR    | dynamic logical partition                   |         |
| DMA      | direct memory access                        | MMA     |
| DME      | dense math engine                           | MPM     |
| DPM      | dynamic performance mode                    | MSP     |
| EA       | effective address                           | MSPP    |
| EDR      | Endpoint Detection and Response             | MTFR    |
| EEH      | Enhanced Error Handling                     | NPIV    |
| EIA      | Electronic Industries Alliance              | NS      |
| ePoE     | electronic Proof of Entitlement             | NUCA    |
| ERAT     | effective-to-real address translation       | NVMe    |
| ESA      | Electronic Service Agent                    | NX      |
| ESM      | Expansion Service Manager                   | OMI     |
| ESS      | Entitled Systems Support                    | ONNX    |
| FFDC     | first-failure data capture                  | OSHA    |
| FHE      | fully homomorphic encryption                | PCIe    |
| FIFO FRU | first in - first out Field-Replaceable Unit | PCR     |

FRU Service Interface

Flexible Service Processor

GNU Compiler Collection giga-transfers per second

hard disk drive half-height, half-length

Hardware Management Console

Hybrid Network Virtualization hardware page table

Infrastructure as a Service

International Business Machines

Corporation idle power saver

Instruction Set Architecture independent software vendor

Keyboard/Video/Mouse

Landscape Management low-profile

logical partition light path diagnostics

Live Partition Mobility memory controller unit

Miscellaneous Equipment

Specification

Matrix Math Accelerator maximum performance mode

Mover Server Partition multiple shared processor pools

MPM typical frequency range

N\_Port ID Virtualization namespace

non-uniform cache access

Non-Volatile Memory Express nest accelerator
...
| PHB   | PCIe host bridge                              | VM      |
|-------|-----------------------------------------------|---------|
| PMIC  | Power Management Integrated Circuit           | VPD VRM |
| PSI   | Processor Support Interface                   | VSU     |
| PSU   | power supply unit                             |         |
| QoS   | quality of service                            | VSX     |
...
virtual local area network virtual machine

vital product data voltage regulator module
